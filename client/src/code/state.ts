...
export function createState(inParentComponent) {
   ...
   return {
       ...
       */
      showEditContact: function (inName: string, inEmail: string): void {

         console.log("state.showEditContact()");

         this.setState({ currentView: "contactEdit", editedContactName: inName, editedContactEmail: inEmail });
      ...
      updateContact: async function (): Promise<void> {
         ...
         console.log("state.updateContact()", this.state.contactID, this.state.editedContactName, this.state.editedContactEmail);

         // update from server.
         this.state.showHidePleaseWait(true);
         const contactsWorker: Contacts.Worker = new Contacts.Worker();
         await contactsWorker.updateContact(this.state.contactID, { name: this.state.editedContactName, email: this.state.editedContactEmail });
         this.state.showHidePleaseWait(false);

         // Remove from list.
         const cl = this.state.contacts.filter((inElement) => {
            if (inElement._id == this.state.contactID) {
               inElement.name = this.state.editedContactName
               inElement.email = this.state.editedContactEmail
            }
         });

         // Update state.
         this.setState({ currentView: "contact", contactID: this.state.contactID, contactName: this.state.editedContactName, contactEmail: this.state.editedContactEmail });
